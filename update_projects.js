...
text = text.replace(/Seventeen projects/g, "Eighteen projects");
text = text.replace(/<strong>17<\/strong>/g, "<strong>18</strong>");
text = text.replace(/№ {project\.num} \/ 17/g, "№ {project.num} / 18");
...
text = text.replace(/const PROJECTS = \[\s*[\s\S]*?\];\s*\nconst SKILLS/m, \`const PROJECTS = [\\n\${newProjectsArray}\\n];\\n\\nconst SKILLS\`);
...
text = text.replace(/const EXPERIENCE = \[\s*[\s\S]*?\];/m, newExperience);
